...
const mapCognitoUser = (attributes: Record<string, any> | null | undefined) => {
  const id = attributes?.sub ?? '';
  const email = attributes?.email ?? '';
  const displayName = attributes?.name ?? email;
  ...
  return buildAuthenticatedUser({
    ...
    username: attributes?.preferred_username ?? attributes?.email ?? null,
    tenantId: attributes?.tenant_id ?? null,
    schoolId: attributes?.school_id ?? null,
    jti: attributes?.jti ?? null,
    iss: attributes?.iss ?? null,
    aud: attributes?.aud ?? null,
    ...attributes,
  });
};
        ...
        if (user) {
          user.getSession(async (err: Error | null, session: CognitoUserSession | null) => {
            if (err) {
              ...
              dispatch({
                type: Types.auth,
                payload: { isAuthenticated: true, user: mapCognitoUser(attributes) },
              });
              resolve({
                user,
                session,
